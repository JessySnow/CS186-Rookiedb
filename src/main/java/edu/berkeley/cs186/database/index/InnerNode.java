...
class InnerNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        // find the child to put new pair
        int i = 0;
        for (; i < keys.size(); i++) {
        ...
        // put the new pair and insert a split key if needed
        BPlusNode child = getChild(i);
        Optional<Pair<DataBox, Long>> splitInfo = child.put(key, rid);
        if (splitInfo.isPresent()) {
            keys.add(i, splitInfo.get().getFirst());
            children.add(i + 1, splitInfo.get().getSecond());
        }
